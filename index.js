...
array.forEach(element => {
  ...
  superagent.get('http://get.fm-kyoto.jp/nowonair?date=' + s_date).use(throttle.plugin()).then((res, err) => {
    if (err) {
      ...
      fs.readFile('songList.json', function (err, data) {
        if(typeof data != "undefined") {
          var json = JSON.parse(data)
        } else {
          var json = []
          fs.appendFile("songList.json",'',(err) => {if (err) throw err;})
        }
        json.push(resultProductData)
        fs.writeFile("songList.json", JSON.stringify(json), (err) => {if (err) throw err;})
      })
    }
  });
});
